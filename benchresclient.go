...
func (c *ResBenchClient) run(res chan *PubResults) {
...
func (c *ResBenchClient) genMessages(ch chan *Message, done chan bool) {
	for i := 0; i < c.MsgCount; i++ {

		topic := c.Aip.AddToTopic(c.PubTopic + "/" + strconv.Itoa(i) + "/HASH")
		...
		//fmt.Println("made a message: " + topic)
	}
	done <- true
...
func (c *ResBenchClient) pubMessages(in, out chan *Message, doneGen, donePub chan bool) {
	onConnected := func(client mqtt.Client) {
		...
		for {
			...
			case m := <-in:
				...
				token.Wait()
				if token.Error() != nil {
					log.Printf("RES-PUBLISHER %v Error sending message: %v\n", c.ID, token.Error())
					m.Error = true
				} else {
			...
			case <-doneGen:
				if !c.Quiet {
					log.Printf("RES-PUBLISHER %v had connected to the broker %v and done publishing for topic: %v\n", c.ID, c.BrokerURL, c.PubTopic)
				}
				donePub <- true
